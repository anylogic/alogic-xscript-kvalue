...
public class KVLRPush extends KVRowOperation {
	protected String item = "";
	protected String delimeter = "";
	
	public KVLRPush(String tag, Logiclet p) {
		super(tag, p);
	}
	...
	protected void onExecute(KeyValueRow row, Map<String, Object> root, Map<String, Object> current,
		...
		if (row instanceof ListRow) {
			...
			if (StringUtils.isNotEmpty(delimeter)){
				ctx.SetValue(id, String.valueOf(r.rightPush(items.split(delimeter))));
			}else{
				ctx.SetValue(id, String.valueOf(r.rightPush(items)));
			}
		}
	}
