...
/**这个命令的作用和 SDIFF 类似，但它将结果保存到 dstKey 集合，而不是简单地返回结果集。</br>
 ...
 *
 */
public class KVSDiffStore extends KVRowOperation {

	protected String others = "";
	protected String dstKey = "";
	protected String delimeter = ";";

	public KVSDiffStore(String tag, Logiclet p) {
		super(tag, p);
	}
	...
	protected void onExecute(KeyValueRow row, Map<String, Object> root, Map<String, Object> current,
		...
		if (row instanceof SetRow) {
			SetRow r = (SetRow) row;
			ctx.SetValue(id, String.valueOf(r.diffStore(dstKey, ctx.transform(others).split(delimeter))));
