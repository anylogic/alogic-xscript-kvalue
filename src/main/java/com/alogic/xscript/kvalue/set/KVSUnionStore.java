...
/**这个命令类似于 SUNION 命令，但它将结果保存到 dstKey 集合，而不是简单地返回结果集。<br/>
 *如果 dstKey 已经存在，则将其覆盖。<br/>
 *dstKey 可以是 key 本身。
 * 
 * @author zhongyi
 *
 */
public class KVSUnionStore extends KVRowOperation {

	protected String others = "";
	protected String dstKey = "";
	protected String delimeter = ";";

	public KVSUnionStore(String tag, Logiclet p) {
		super(tag, p);
	}
	...
	protected void onExecute(KeyValueRow row, Map<String, Object> root, Map<String, Object> current,
		...
		if (row instanceof SetRow) {
			SetRow r = (SetRow) row;
			ctx.SetValue(id, String.valueOf(r.unionStore(dstKey, null,ctx.transform(others).split(delimeter))));
		}
	}
